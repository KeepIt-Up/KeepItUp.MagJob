...
To update the database use this command from the Web project folder (replace `KeepItUp.MagJob.Identity` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../KeepItUp.MagJob.Identity.Infrastructure/KeepItUp.MagJob.Identity.Infrastructure.csproj -s KeepItUp.MagJob.Identity.Web.csproj
```
